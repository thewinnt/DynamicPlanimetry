...
public class Notifications extends Actor implements Disposable {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        for (int i = 0; i < NOTIFICATIONS.size(); i++) {
            ...
            drawer.sector(x1, y, 10, (float)Math.toRadians(90), (float)Math.toRadians(90), GuiTheme.current().button(), GuiTheme.current().button());
            drawer.sector(x2, y, 10, 0, (float)Math.toRadians(90), GuiTheme.current().button(), GuiTheme.current().button());
            drawer.sector(x1, y - cache.height - 10, 10, (float)Math.toRadians(180), (float)Math.toRadians(90), GuiTheme.current().button(), GuiTheme.current().button());
            drawer.sector(x2, y - cache.height - 10, 10, (float)Math.toRadians(270), (float)Math.toRadians(90), GuiTheme.current().button(), GuiTheme.current().button());
            drawer.filledRectangle(x1 - 10, y - cache.height - 10, 10, cache.height + 10, GuiTheme.current().button());
            drawer.filledRectangle(x2, y - cache.height - 10, 10, cache.height + 10, GuiTheme.current().button());
            drawer.filledRectangle(x1, y, x2 - x1, 10, GuiTheme.current().button());
            drawer.filledRectangle(x1, y - cache.height - 20, x2 - x1, 10, GuiTheme.current().button());
            drawer.filledRectangle(x1, y - cache.height - 10, x2 - x1, cache.height + 10, GuiTheme.current().button());
    ...
    public void updateCaches() {
        caches.clear();
        float y = Gdx.graphics.getHeight() - 20;
        BitmapFont font = this.font.getFont(Size.MEDIUM.lines(1), GuiTheme.current().textButton());
        for (Notification i : NOTIFICATIONS) {
            this.layout.setText(font, i.entry, 0, i.entry.length(), GuiTheme.current().textButton(), getWidth() - 40, Align.center, true, null);
