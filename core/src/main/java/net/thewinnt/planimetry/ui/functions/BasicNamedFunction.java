...
public class BasicNamedFunction<T extends Shape> extends Function<T> {
    ...
    public BasicNamedFunction(Drawing drawing, T shape, Consumer<T> action, Component name, String actionButton) {
        super(drawing, shape);
        this.action = action;
        this.name = name;
    ...
    public WidgetGroup setupActors(StyleSet styles) {
        ...
        TextButton apply = new TextButton(actionButton, styles.getButtonStyle(Size.MEDIUM, true));
