package net.thewinnt.planimetry.definition.point.type;

import net.querz.nbt.tag.CompoundTag;
import net.thewinnt.planimetry.data.Drawing;
import net.thewinnt.planimetry.data.LoadingContext;
import net.thewinnt.planimetry.data.NbtUtil;
import net.thewinnt.planimetry.data.SavingContext;
import net.thewinnt.planimetry.definition.point.PointPlacement;
import net.thewinnt.planimetry.definition.point.PointPlacementType;
import net.thewinnt.planimetry.definition.point.ValueContext;
import net.thewinnt.planimetry.definition.point.placement.MousePlacement;

import org.jetbrains.annotations.Nullable;

import java.util.Random;

public class MousePlacementType implements PointPlacementType<MousePlacement> {
    public static final MousePlacementType INSTANCE = new MousePlacementType();

    private MousePlacementType() {}

    @Override
    public MousePlacement fromNbt(CompoundTag nbt, LoadingContext context) {
        return null;
    }

    @Override
    public CompoundTag writeNbt(PointPlacement value, SavingContext context) {
        CompoundTag nbt = new CompoundTag();
        NbtUtil.writeVec2(nbt, value.get(), "position");
        return nbt;
    }

    @Override
    public MousePlacement convert(PointPlacement other, Drawing drawing) {
        return new MousePlacement();
    }

    @Override
    public @Nullable MousePlacement createRandom(Random random, ValueContext context) {
        return new MousePlacement();
    }
}
